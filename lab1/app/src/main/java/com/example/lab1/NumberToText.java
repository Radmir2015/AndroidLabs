...
public class NumberToText {
    private final static int DIGIT_POWER = 6;

    private final static String[][] PowersAndCases = new String[][] {
            {"0", ""            , ""             ,""              },  // 1
    ...
    private final static String[][] DigitsAndCases = new String[][] {
            {""       ,""       , "десять "      , ""            ,""          },
    ...
    public static String convertToText(int sum) {
        int i, mny;
        ...
        for (i = 0, divisor = 1; i < DIGIT_POWER; i++) divisor *= 1000;

        for (i = DIGIT_POWER - 1; i >= 0; i--) {
            divisor /= 1000;
            ...
            if (mny == 0) {
                ...
                if (mny >= 10) {
                    ...
                    if (mny >= 1) result.append(DigitsAndCases[mny]["0".equals(PowersAndCases[i][0]) ? 0 : 1]);
                }
                switch (mny) {
                    case 1:
                        result.append(PowersAndCases[i][one]);
                    ...
                    case 3:
                    case 4:
                        result.append(PowersAndCases[i][four]);
                        break;
                    default:
                        result.append(PowersAndCases[i][many]);
                        break;
                };
            }
        }
    ...
    public static String convertToText(double num) {
        return convertToText((int) num) + "." + convertToText((int)(num * 100 - ((int) num) * 100));
    }
}